...
def load_or_download_model(local_path, model_name, subfolder=None, variant=None, quantization_config=None):
    if os.path.exists(local_path):
        print(f"Loading model from {local_path}...")
        model = T5EncoderModel.from_pretrained(local_path)
        # model = T5EncoderModel.from_pretrained(local_path, torch_dtype=torch.float16)
        # model = model.to("cuda")
        print(model.device)
    else:
...
def measure_wall_clock_time(model, seq_len=77, device="cpu"):
    # model = model.to(device)
    input_ids = torch.randint(0, 32100, (1, seq_len))
...
if __name__ == "__main__":
    if True:
        ...
        local_model_path = "./saved_models/t5_encoder_3_quantized"
        model_name = "stabilityai/stable-diffusion-3-medium-diffusers"
        subfolder = "text_encoder_3"
        variant = None
    ...
    t5 = load_or_download_model(local_model_path, model_name, subfolder=subfolder, variant=variant, quantization_config=quantization_config)
    ...
    measure_wall_clock_time(seq_len=2, model=t5)
